...
## Deferred from: code review of 6-5-migration-backfill-and-operator-recovery (2026-05-06)

- Dead `continue` in `backfill_legacy_documents` inner version loop — fires after all UPDATEs/INSERTs are done; nothing follows it in the inner loop body so it skips nothing; remove or restructure the guard for clarity [`src/cos/store/db.py:256–261`]
- `cos migrate` bypasses `_repair_existing_schema` — `_run_migrate` opens a direct psycopg connection without calling `run_migrations`; on a database where the `source_versions_source_document_unique` constraint is missing, the `ON CONFLICT ON CONSTRAINT` clauses will fail with an unknown-constraint error; sequence: start the MCP server once first to trigger `run_migrations` before running `cos migrate` [`src/cos/cli.py`]
