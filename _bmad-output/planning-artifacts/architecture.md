...
Epic 8 adds reactive Telegram messaging. Future agents should treat all of the following as implemented baseline.

| # | Deviation / Addition | Detail |
|---|----------------------|--------|
| 1 | **`telegram-bot` service added to Docker Compose** | Epic 8 adds a fifth Docker Compose service: `telegram-bot` (image: same Dockerfile, entrypoint: `uv run cos-telegram-bot`). It runs `src/cos/connectors/telegram_bot.py` as its persistent process and long-polls the Telegram Bot API using `getUpdates`. It shares `data/`, `config.yaml`, `role_packs/`, `tokens/`, and `local/certs/` volume mounts with the `cos` and `worker` services. It exits cleanly at startup when `"telegram"` is not in the `connectors:` list or the `telegram:` config block is absent — the service is defined in `docker-compose.yml` unconditionally but is only useful when Telegram is configured. Use `docker compose logs telegram-bot` to confirm it is polling. |
| 2 | **Connector config model: `connectors:` list + top-level block** | `CosConfig` (in `src/cos/config.py`) has `connectors: list[str]` as the activation gate and an optional top-level `telegram: TelegramConnectorConfig` block for Telegram settings. There is no nested `connectors.telegram` object. `TelegramConnectorConfig` fields: `bot_token`, `chat_id`, `api_base_url`, `poll_timeout`, `backoff_initial`, `backoff_max`, `staging_dir`. The same pattern applies to `gmail:` and `google_calendar:` blocks. |
...
| 4 | **Telegram is reactive-only in Epic 8** | The bot classifies inbound text as `question`, `note`, or `unsupported`. Questions trigger `RetrievalService.query` and return a concise cited reply through `OutputService → TelegramChannel`. Notes prefixed with `Note:` are normalised, staged to `staging_dir`, and submitted as `telegram_note` ingest jobs for the `worker` to process through the canonical pipeline. Bare greetings, unknown commands, non-text messages, and empty `Note:` messages are ignored with no reply. **Proactive delivery (morning briefs, scheduled digests, meeting prep) is not part of Epic 8.** Those belong to Epic 11. |
...
| 6 | **Deduplication at enqueue time** | A Telegram note that matches an already-queued or already-processed content hash returns `"Note saved."` without creating a duplicate ingest job or canonical record. This is consistent with the four deterministic ingest outcomes (`new_content`, `unchanged`, `changed_content`, `new_source_known_content`) established in Epic 6. |
