...
Before you seed Gmail or MCP, create one shared exact-byte artifact on the host:

```bash
mkdir -p test-docs
printf '%s' 'Epic 6 UAT note. This note tracks workforce planning for the quarterly board review. Marker: epic-6-uat-note-a.' > test-docs/epic-6-uat-shared-note.md
```

Important:

- use `printf`, not `echo`, so the file has no trailing newline
- do not edit this file after creating it
- attach this exact file to both Gmail test emails below
- keep the MCP Note A content in Section 7.1 byte-identical to this file
...
2. Send yourself an email with:
   ...
   - attach `test-docs/epic-6-uat-shared-note.md`
...
3. Send yourself a second email with:
   ...
   - attach the same `test-docs/epic-6-uat-shared-note.md` bytes again
   - the `cos-uat` label applied
...
Use the bundled test docs plus the shared exact-byte artifact you created above:
...
- `cos docs --json` returns objects with:
  ...
  - `current_version`
  - `chunk_count`
- `epic-6-uat-shared-note.md` appears as a local `file` source after this ingest
...
```text
Call ingest_document with the same content and the same metadata as Section 7.1, except set:
- metadata.external_id: "epic-6-uat-note-002"
Keep:
- metadata.title: "Epic 6 UAT Note"
...
Now verify the full cross-source dedupe proof using SQL — the local file, Gmail attachment, and MCP note should share one canonical document and content blob:
...
JOIN document_versions dv ON dv.id = sv.document_version_id
WHERE
  (s.source_type = 'file' AND s.source_locator LIKE '%/epic-6-uat-shared-note.md')
  OR (s.source_type = 'gmail_attachment' AND s.source_alias = 'epic-6-uat-shared-note.md')
  OR (s.source_type = 'mcp_note' AND s.source_locator = 'mcp_note://claude-code/epic-6-uat-note-001')
...
- at least three rows: one `file`, one `gmail_attachment`, and one `mcp_note`
- all rows have the **same** `sha256` and `document_id`
- all rows have **different** `source_type` / `source_locator` values

If you also attached the same file to both Gmail test emails, you may see more than one `gmail_attachment` row here. That is fine as long as all of them still share the same `sha256` and `document_id`.

This proves one byte-identical artifact survives as three distinct provenance identities while collapsing to one shared canonical content record.
...
Capture a UTC timestamp before you re-run the syncs:
...
- Gmail sync output shows messages scanned
- Calendar sync output shows calendars scanned
- job counts may be non-zero because connectors rescan and enqueue work before canonical dedupe runs in the worker
...
- immediately after the syncs, you may see newly created `queued` or `running` jobs
- after the worker catches up, there should be no long-lived `queued` or `running` backlog for jobs created since `RESTART_SYNC_STARTED_AT`
- most or all post-restart jobs should settle to `succeeded`
...
- the Section 7.3 SQL query shows the local file, Gmail attachment, and MCP note rows sharing one `sha256` and `document_id`
...
- post-restart jobs created since `RESTART_SYNC_STARTED_AT` drain back to no long-lived backlog
